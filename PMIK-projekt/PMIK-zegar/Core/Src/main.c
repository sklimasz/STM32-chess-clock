...
/**
  ...
  * @file           : main.c
  * @brief          : Chess clock
  * @authors		: Szymon Klimaszewski, KajetanLachanski
  * @description	:
  *
  * Chess clock using MAX7219 8*7-segment display,
  * switch-buttons, SD card for game data storage
  * and ESP32 for wifi data transfer.
  *
  ******************************************************************************
  */
...
int main(void)
{
  ...
  /**************************************
   Example SD-Card saving (init-only)

  HAL_Delay(500);
  f_mount(&fs, "", 0);
  f_open(&fil, "write.txt", FA_OPEN_ALWAYS | FA_WRITE | FA_READ);
  f_lseek(&fil,f_size(&fil)); //change
  f_puts("Chess clock init\n", &fil);
  f_close(&fil);
